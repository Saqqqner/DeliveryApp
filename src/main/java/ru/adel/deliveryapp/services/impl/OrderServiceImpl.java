...
public class OrderServiceImpl implements OrderService {
    ...
    @Autowired
    public OrderServiceImpl(OrderRepository orderRepository, CustomerServiceImpl customerServiceImpl, ProductServiceImpl productServiceImpl, OrderItemRepository orderItemRepository) {
...
//    Создание заказа
    public Order createOrder(OrderDTO orderDTO) {
        ...
        order = orderRepository.save(order);  // Сохраните заказ, чтобы получить его идентификатор
        for (OrderItem orderItem : orderItems) {
            orderItem.setOrder(order);  // Установите правильное значение для поля "order"
        }
